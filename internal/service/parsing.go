...
注意：用户输入可能不是完整句子，可能是零散的词语或短语堆叠（如"泵车 恒大 33米 15方"），请根据关键词判断类型。

类型说明：
- pump_truck: 泵车记账。关键词包括泵车、泵、臂架、泵送、XX米（车型）等
- mixer_truck: 搅拌车记账。关键词包括搅拌车、搅拌、混凝土运输、罐车、多个驾驶员各拉几方等
...
注意：用户输入可能不是完整句子，可能是零散的词语或短语堆叠（如"33米 恒大 15方 XX工地"），请根据语义推断各字段。
...
如果某必填字段无法从输入中提取，仍设为 null，但请尽量从上下文中推断。

输出示例：
{"date":"2026-03-07","truck_model":"红泵车","customer_name":"XX建设","volume":15.0,"location":"XX工地"}`

const mixerTruckSystemPrompt = `你是搅拌车施工记录的数据提取助手。请从用户提供的文字或图片中提取以下字段，严格只返回 JSON，不要返回任何其他内容。

注意：用户输入可能不是完整句子，可能是零散的词语或短语堆叠（如"恒大 张三8+7 李四6+5"），请根据语义推断各字段。

字段说明：
- date: 【必填】日期，格式 "YYYY-MM-DD"
- customer_name: 【必填】客户名称
- volume: 【必填】方量，数字类型（单位：立方米），即所有驾驶员运送方量的总和
- drivers: 【必填】驾驶员姓名列表，数组类型
- remark: 【必填】备注，记录各个驾驶员每车运送的方量明细，原样提取用户描述的内容即可

如果某必填字段无法从输入中提取，仍设为 null（drivers 设为空数组 []），但请尽量从上下文中推断。

输出示例：
{"date":"2026-03-07","customer_name":"XX建设","volume":31.0,"drivers":["张三","李四"],"remark":"张三：8+7+6，李四：5+5"}`
...
func (s *ParsingService) ParsePumpTruck(ctx context.Context, input ParseInput) (*PumpTruckParseResult, error) {
	...
	// 拉取泵车表字段选项，增强提示词
	optionsHint := s.fetchFieldOptionsHint(ctx, s.bitableCfg.PumpTruckTableID, []string{"车型", "客户名称"})
	...
	log.Printf("[Parsing] 泵车记录解析成功: date=%v model=%s customer=%s volume=%.1f location=%s",
		record.Date.Format("2006-01-02"), record.TruckModel, record.CustomerName, record.Volume, record.Location)
...
func (s *ParsingService) ParseMixerTruck(ctx context.Context, input ParseInput) (*MixerTruckParseResult, error) {
	...
	log.Printf("[Parsing] 搅拌车记录解析成功: date=%v customer=%s volume=%.1f drivers=%v remark=%s",
		record.Date.Format("2006-01-02"), record.CustomerName, record.Volume, record.Drivers, record.Remark)
